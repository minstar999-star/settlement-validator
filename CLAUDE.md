...
- OLAP 엑셀 원본은 브라우저 안에서만 읽고 처리하며 서버로 올리지 않는다. 질의서 PDF는 iOS Safari 호환 문제로 텍스트 추출만 서버(`/api/extract-pdf`)를 거치되, 저장하지 않고 요청 처리 중에만 메모리에 둔다(2026-08-04 변경, 자세한 배경은 `PRD.md` 7번). 서버로 보내는 것은 이 PDF 원본과, 불일치 원인 분석에 필요한 최소 텍스트·수치로 한정한다.
